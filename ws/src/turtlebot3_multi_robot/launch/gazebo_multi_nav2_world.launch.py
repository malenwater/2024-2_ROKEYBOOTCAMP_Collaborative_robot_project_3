...
'''
gazebo에서 다중turtlebot3로봇을 실행하고
nav2을 통해 네비게이션을 관리하는 ros2런치 스크립트
...
def generate_launch_description():
    ...
    # gazebo시뮬레이터 실행을 위한 gzserver, gzclient실행
    ...
    for robot in robots:
        ...
        # 터틀봇3 을 순차적으로 gazebo에 스폰하고, 네이베이션을 설정하는 과정
        # -> 병렬 실행 문제를 방지하기 위한 코드
        if last_action is None:
            ...
            # RegisterEventHandler를 사용하여 이전 로봇이 완전히 스폰된 후 실행
            # 이전 로봇이 생성된 후 다음 로봇을 실행하도록 설정
            spawn_turtlebot3_event = RegisterEventHandler(
                event_handler=OnProcessExit(
                    target_action=last_action,  # 이전 로봇의 생성 완료를 기다림
                    on_exit=[spawn_turtlebot3_burger,   # 새로운 로봇 스폰
                            turtlebot_state_publisher,  # 새로운 로봇의 상태 퍼블리셔 실행
                            bringup_cmd],  # Nav2 네비게이션 실행
                )
            )
